...
- [x] 1. Backend — Serviço e contrato de desvinculação em lote
  - [x] 1.1 Adicionar método `detachAllTransactions(string $periodUid, string $userUid): int` ao `PeriodServiceInterface`
    - Arquivo: `app/Domain/Period/Contracts/PeriodServiceInterface.php`
    - _Requisitos: 4.3, 4.4_
  - [x] 1.2 Implementar `detachAllTransactions` no `PeriodService`
    ...
    - _Requisitos: 4.3, 4.4_
  - [x] 1.3 Adicionar eager loading de `account` e `category` em `getTransactionsForPeriod`
    - Adicionar `->with(['account', 'category'])` ao query existente
    - Arquivo: `app/Domain/Period/Services/PeriodService.php`
    - _Requisitos: 2.1_

- [x] 2. Backend — Controller, rotas e validação
  - [x] 2.1 Adicionar `period_uid` ao `StoreTransactionRequest`
    ...
    - _Requisitos: 5.8_
  - [x] 2.2 Atualizar `show()` no `PeriodPageController` para incluir `accounts` e `categories` como props
  ...
  - [x] 2.3 Implementar método `storeTransaction` no `PeriodPageController`
  ...
  - [x] 2.4 Implementar método `detachTransactions` no `PeriodPageController`
    - Chamar `PeriodService::detachAllTransactions()`
    - Redirecionar com flash success incluindo contagem
    - Incluir `try-catch` com `Log::error()` e flash error
    - Arquivo: `app/Domain/Period/Controllers/PeriodPageController.php`
    - _Requisitos: 4.3, 4.5, 4.6_
  - [x] 2.5 Registrar novas rotas em `Period/Routes/web.php`
...
- [x] 4. Backend — Testes de feature (PHPUnit)
  - [x] 4.1 Criar teste `PeriodTransactionManagementTest` com cenários de detach
  ...
  - [x] 4.2 Adicionar cenários de store transaction ao mesmo arquivo de teste
  ...
  - [x] 4.3 Escrever teste de propriedade: Detach preserva existência das transações
  ...
  - [x] 4.4 Escrever teste de propriedade: Criação com period_uid vincula corretamente
  ...
  - [x] 4.5 Escrever teste de propriedade: Validação de period_uid aceita UUIDs válidos e rejeita strings inválidas
...
- [x] 5. Checkpoint — Validar testes backend
  - Rodar `php artisan test --compact tests/Feature/PeriodTransactionManagementTest.php`
  - Ensure all tests pass, ask the user if questions arise.

- [x] 6. Frontend — Schema, form e sidebar
  - [x] 6.1 Adicionar `period_uid` ao Zod schema de transação
    ...
    - _Requisitos: 5.8_
  - [x] 6.2 Adicionar props `periodUid?` e `periodDate?` ao `TransactionForm.vue`
  ...
  - [x] 6.3 Reordenar item "Períodos" na sidebar para 2ª posição
    - Mover `{ title: 'Períodos', ... }` para índice 1 no array `financeNavItems`
    - Arquivo: `resources/js/components/AppSidebar.vue`
    - _Requisitos: 1.1, 1.2, 1.3_

- [x] 7. Frontend — Página Show.vue com agrupamento, criação e remoção
  - [x] 7.1 Adicionar props `accounts` e `categories` ao `Show.vue`
    - Atualizar `defineProps` com `accounts: Account[]` e `categories: Category[]`
    - _Requisitos: 5.4_
  - [x] 7.2 Implementar agrupamento por INFLOW/OUTFLOW com subtotais
  ...
  - [x] 7.3 Adicionar botão "Nova Transação" com ModalDialog e TransactionForm
  ...
  - [x] 7.4 Adicionar botão "Remover Todas as Transações" com AlertDialog de confirmação
...
- [x] 8. Frontend — Gerar rotas Wayfinder e validar build
